import { editFurnitureDetails } from '../data/requests.js';
import { notificationHandler } from './notifications.js';
import { redirect } from './redirect.js';

export const editFurniture = async (e, ctx) => {
    e.preventDefault();
    ...
    try {
        const response = await editFurnitureDetails(ctx.details._id, {
            method: 'PUT',
            headers: {
        ...
        notificationHandler('Successfully edited!', 'success');
        redirect('/');
    } catch (error) { return; }
}